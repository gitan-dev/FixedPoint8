...
public class FixedPoint8Tests
{
    ...
    public void BufferSizeTest()
    {
        ...
        //char
        var fp8 = FixedPoint8.One;
        for (int i = 1; i <= 11; i++)
        {
            Span<char> buffer = new char[i];

            result = fp8.TryWriteChars(buffer, out charsWritten);
            Assert.IsTrue(result);
            Assert.IsTrue(charsWritten == i);
        ...
        for (int i = 1; i <= 11; i++)
        {
            Span<byte> buffer = new byte[i];

            result = fp8.TryWriteUtf8(buffer, out charsWritten);
            Assert.IsTrue(result);
            Assert.IsTrue(charsWritten == i);
    ...
    {
        var value = FixedPoint8.Zero;
        IFormattable a = value;
        IUtf8SpanFormattable b = value;
        IUtf8SpanParsable<FixedPoint8> c = value;
        ISpanParsable<FixedPoint8> d = value;
        ISpanFormattable e = value;
        INumber<FixedPoint8> f = value;
        ISignedNumber<FixedPoint8> g = value;
        IMinMaxValue<FixedPoint8> h = value;
    }
}
